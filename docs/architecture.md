...
- Under 721px the Explorer takes a different shape and drops nothing. The toolbar keeps the search field alone; the selects move into `components/FilterPanel.tsx`, a full-screen boxed dialog opened by the FILTERS button on the results line, whose badge counts the select filters only (never the search query, which stays visible in its own field, and never a route-locked language). The fields are one component, `components/FilterFields.tsx`, mounted twice with an id prefix per mount, so the document never carries two `#f-family`. The panel is a real dialog: role, aria-modal, its name from the panel title, initial focus inside, hand-rolled focus containment (no dependency), Escape and a close button, focus restored to the trigger, and a body scroll lock that saves the prior overflow value and restores it on close and on unmount. A window widened past the breakpoint closes the panel rather than leaving a modal over the desktop toolbar. Rows go two-line (`components/VoiceRow.tsx`): the name on the first, and the family tag, the effective sub-model, the styles, the trait tag and the gender on the second, indented to the name's edge because the play button's tap target spans both lines while its 32px look is unchanged. A transient generating or error note takes the metadata line and the metadata returns when it clears. Every rule of this shape lives in a media query: from 721px up the Explorer is exactly what it was.
