...
The owner lifted the hold: the domain and mailboxes are being secured, so the held contact item shipped. Address chosen jointly with the peer reviewer and locked by the owner: hi@speakshelf.com, matching the site's human copy voice; info@ was rejected as the most dictionary-guessed alias with the coldest register, support@ as overpromising a desk. The launch shape from the hold entry shipped as recorded: a visible, copyable mailto address in the footer's right utility column, labeled per locale, held LTR so it never reorders in Arabic. An unpublished info@ alias into the same box stays an option on the mail side, never on the site. Mailbox setup notes passed to the owner: SPF, DKIM and DMARC before sending, replies always from the domain address.
