...
The hero subtitle and the home JSON-LD now follow the share-card pattern the owner set: the first three shelves as a bare comma list, with each locale's message closing the list itself ("and more", "y más", "等") and starting a fresh sentence after. The counted "and N more" variant is gone. The list joins in listNamesPlain with a plain comma, enumeration comma for zh; Intl.ListFormat was rejected there because its unit type concatenates zh names with no separator and its conjunction styles double the message's own closing conjunction in es and hi.
