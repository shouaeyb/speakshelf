...
Masthead (sticky, 48px) with global provider tabs (active provider underlined in blue, short labels under 560px), hero with stat row and mono section jump links, family tiles (grid columns follow the tile count below five, so sparse providers show no dead cells), umbrella provider cards (`.prov-card`: mono `/KEY`, name, big count, mono meta, blurb, BROWSE line), the umbrella "why" tiles, dark language band, boxy toolbar (5, 4 or 3 fields; the family filter label uses the provider's own word), voice rows with play button states (play, spinner, equalizer, error note), sticky language group heads, footer. Two floating Carbon notifications share one shape and one entrance: the family toast on the page's light layer, and the language suggestion on the inverse dark ground that marks it as chrome rather than content. Neither sits in the page. The suggestion is a card in the top inline-end corner on desktop and spans the width under the masthead below 521px; it never occupies space in the flow, because it is decided after the page has already painted and a bar that appears late pushes the whole page down. Anything else that appears after load follows that rule and takes an explicit place in the overlay stack recorded in `docs/architecture.md`. Reuse these before inventing anything.
